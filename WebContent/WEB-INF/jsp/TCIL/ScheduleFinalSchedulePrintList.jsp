...
<c:choose>
          <c:when test="${command.checkday=='selected'}">
            <center><h3><b><c:out value="${command.finalschedule.day}"/> &nbsp;<fmt:message key="schedule.heading.Schedule"/></b></h3></center>
          </c:when>
          <c:otherwise>
...
<c:choose>
 ...
 <td class="center"><c:out value="${status.count}"/></td>
				        ...
				        <td class="left"><c:out value="${schedulelist.peHospital.hospitalName}"/></td>
