...
<script language="javascript">
    ...
    function addFinalSchedule(opt)  //Calling for Adding a new Schedule
     {
         document.ScheduleChart.action="AddSchedule.htm";
	     document.ScheduleChart.submit();
     }

    function backFinalSchedule()  //back
     {
         document.ScheduleChart.action="Tcil.htm";
	     document.ScheduleChart.submit();
     }
    ...
    function deleteFinalSchedule(opt)  //calling the controller for delete option
     {
      if(confirm("Are You Sure,You Wan't To Delete Selected Schedules."))
	   {
		document.ScheduleChart.action="Tcil.htm?buttonsd="+opt;
	    document.ScheduleChart.submit();
	   }
     }
    ...
    function results(opt)
     {
      document.forms[0].action="Tcil.htm?search=true&buttonsd=Search&page="+opt;
...
<div id="Card" style="display:none">
	<jsp:include page="/WEB-INF/jsp/TCIL/ScheduleFinalSchedulePrintList.jsp"/>
</div>
<div id="AddedMessage"> 
<form name="ScheduleChart" method="Post">
<c:set var="mainSelected" value="mmHome" scope="session"/>
<c:set var="subSelected" value="smfinalSchedule" scope="session"/>
  <jsp:include page="/WEB-INF/jsp/menu/MainMenu.jsp"/>
  
  <tr>
    <td colspan="2" class="h2 center"><fmt:message key="schedule.heading.Name"/></td>
  </tr>
</table>
<br>
      ...
      <tr class="read">
        <td align="CENTER"><fmt:message key="schedule.SSH.Hospital"/><strong>:</strong> &nbsp;
           <spring:bind path="command.finalschedule.sshHospital.hospitalId">
		      <select name="<c:out value="${status.expression}"/>" id="sshid">
          ...
          &nbsp;<fmt:message key="schedule.PE.Hospital"/><strong>: &nbsp;</strong>
             <spring:bind path="command.finalschedule.peHospital.hospitalId">
		       <select name="<c:out value="${status.expression}"/>" id="peid">
                   <option value=''><fmt:message key="schedule.AllPE.Select"/></option>
                   <c:forEach var="patient" items="${command.patientId}" varStatus="status">
                     <option value='<c:out value="${patient.hospitalId}"/>' <c:if test="${patient.hospitalId == command.finalschedule.peHospital.hospitalId}"> selected </c:if>> <c:out value="${patient.hospitalName}"/></option>
                   </c:forEach>
               </select>
             </spring:bind>
  ...
  <TABLE width="98%" border="0" align=center cellPadding=0 cellSpacing=0 class="table_bg">
   <c:choose>
    <c:when test="${command.errormessage=='DNE'}">
     ...
     <p class="center">
     <input name="buttonsd" type="button" OnClick="javascript:document.location.href='AddSchedule.htm';" class="button" value=" <fmt:message key="button.Add.Admin"/> "/>
     </p>
   </c:when>
  ...
  <c:otherwise>
   ...
   <c:set var="scheduleCount" value="0" scope="page"/>
     ...
     <TD colspan="3"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_list">
      <c:choose>
        <c:when test="${command.checkday=='selected'}">
       <tr class="left"><th width="2%">&nbsp;</th><th><fmt:message key="schedule.SSH.Hospital"/></th><th><fmt:message key="schedule.PE.Hospital"/></th><th align="CENTER"><fmt:message key="schedule.Specialty"/></th><th align="CENTER"><fmt:message key="schedule.FromTime"/></th><th align="CENTER"><fmt:message key="schedule.ToTime"/></th><th align="CENTER"><fmt:message key="schedule.Edit"/></th></tr> 
      </c:when>
      <c:otherwise>
      <tr class="left"><th width="2%">&nbsp;</th><th><fmt:message key="schedule.SSH.Hospital"/></th><th><fmt:message key="schedule.PE.Hospital"/></th><th align="CENTER"><fmt:message key="schedule.Specialty"/></th><th align="CENTER"><fmt:message key="schedule.FromTime"/></th><th align="CENTER"><fmt:message key="schedule.ToTime"/></th><th align="CENTER"><fmt:message key="schedule.Day"/></th> <th align="CENTER"><fmt:message key="schedule.Edit"/></th></tr> 
      </c:otherwise>
      </c:choose>
           ...
           <c:forEach var="schedulelist" items="${command.pageListHolder.pageList}" varStatus="status">
              <c:choose>
                 <c:when test="${status.count % 2 == 0}">
                  ...
                  <td><c:out value="${schedulelist.sshHospital.hospitalName}"/></td>
                  <td><c:out value="${schedulelist.peHospital.hospitalName}"/></td>
                  ...
                  <td align="CENTER"><a href="EditSchedule.htm?scheduleId=<c:out value="${schedulelist.scheduleId}"/>"><fmt:message key="schedule.Edit"/></a></td>
           </c:forEach> 
           <c:choose>
   ...
   <c:when test="${command.errormessage=='NotDeleted'}">
    <TR>
     ...
     <c:otherwise>
         ...
         <c:when test="${command.errormessage=='Deleted'}">
     <TR>
       <TD class="read bold" colspan="8" align="center"><font color="darkgreen">&quot;<fmt:message key="schedule.delete.Success"/>&quot;</font></TD>
   ...
   <c:when test="${command.errormessage=='NotDeleted'}">
    <TR>
      <TD class="read bold" colspan="8" align="center"><font color="darkgreen">&quot;<fmt:message key="schedule.delete.Notsuccess"/>&quot;</font></TD>
...
<TR>
  ...
  <input type="submit" class="button" name="chk_Print" value="Print"> </td>
	</TD>
